...
// Define multiple disaster-related keywords
const searchKeywords = ['#flood', '#earthquake', '#cyclone', '#fire', '#tsunami']; // Add more keywords as needed
...
async function fetchTweets(searchKeyword) {
    try {
        ...
        if (filteredTweets.length > 0) {
            convertToCSV(filteredTweets, searchKeyword);  // Include keyword in CSV
        } else {
            console.log(`No new tweets found for '${searchKeyword}' within the last 120 minutes.`);
        }

        return tweets;

    } catch (error) {
        console.error(`Error fetching tweets for '${searchKeyword}':`, error.response ? error.response.data : error.message);
    }
}

// Function to convert JSON data to CSV and write it to a file
function convertToCSV(data, keyword) {
    try {
        ...
        const savePath = path.join('D:', 'web_dev', 'nodejs', 'disaster_aggregation_api_backend', `${keyword}_disaster_tweets.csv`);

        // Write the CSV data to a file
        fs.writeFileSync(savePath, csv, { flag: 'a' });  // 'a' flag to append to the file
        console.log(`CSV file updated for '${keyword}': ${savePath}`);
    } catch (err) {
